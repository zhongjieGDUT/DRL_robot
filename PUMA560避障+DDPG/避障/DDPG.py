"""
Environment is a Robot Arm Build in matlab2019a
...
def train():
    ...
    for ep in range(MAX_EPISODES):
        ...
        for t in range(MAX_EP_STEPS):
            ...
            if t == MAX_EP_STEPS - 1 or done or danger:
                # if done:
                result = '| done' if done else '| ----', 'danger' if danger else '---'
                print('Ep:', ep,
                      result,
                      '| R: %i' % int(ep_reward),
                      '| Explore: %.2f' % var,
...
def eval():
    ...
    while True:
        ...
        while True:
            ...
            s_, r, done, danger = env.step(a)
            time.sleep(0.01)
            temp=np.array(env.arm_info)
            q = np.row_stack((q,temp))
            s = s_
            if done:
                print('done')
                break
